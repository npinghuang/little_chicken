...
pose_tracker::pose_tracker(){
  ...
  n.param<float>("xy_tolerance", xy_tolerance_default, 0.01);
  n.param<float>("theta_tolerance", theta_tolerance_default, 0.01);
  ...
  n.param<float>("v_dacc_tolerance", v_dacc_tolerance, 0.02);
  ...
  n.param("tolerance_input", tolerance_input, tolerance_input);
  ...
  ROS_INFO_STREAM("type : "<< tolerance_input.getType());
  for(int i=0 ; i<tolerance_input.size() ; i++ ){
    Tolerance tolerance;
    XmlRpc::XmlRpcValue member = tolerance_input[i];
    tolerance.number = member[0];
    tolerance.xy_tolerance = double(member[1]);
    tolerance.theta_tolerance = double(member[2]);

    tolerance_list.push_back(tolerance);
    ROS_INFO_STREAM("Number : "<< tolerance.number );
    ROS_INFO_STREAM("XY_tolerance : "<< tolerance.xy_tolerance );
    ROS_INFO_STREAM("Theta_tolerance : "<< tolerance.theta_tolerance );
  }
...
void pose_tracker::goalCB(const geometry_msgs::PoseStamped::ConstPtr &goal){
  ...
  xy_tolerance = xy_tolerance_default;
  theta_tolerance = theta_tolerance_default;
  for(int i=0;i<tolerance_list.size();i++){
    if(plan_number == tolerance_list[i].number){
      xy_tolerance = tolerance_list[i].xy_tolerance;
      theta_tolerance = tolerance_list[i].theta_tolerance;
    }
  }
  ROS_INFO_STREAM("Plan number : "<< plan_number);
  ROS_INFO_STREAM("tolerance : "<<xy_tolerance);
}
...
bool pose_tracker::reach_goal_xy(float g_x, float g_y, float n_x, float n_y){
  if( (abs(g_x - n_x) <= xy_tolerance) && (abs(g_y - n_y) <= xy_tolerance) ){
    ROS_INFO_STREAM("robot x : "<<n_x);
    ROS_INFO_STREAM("robot y : "<<n_y);
    ROS_INFO_STREAM("desire x : "<<g_x);
    ROS_INFO_STREAM("desire y : "<<g_y);
    return true;
  }
  else{
    return false;
...
void pose_tracker::emergency_dec(){
  ...
  bool canEmergency = false;
  if(fabs(w_now)>=w_threshold){
    if(w_now>=0)
  ...
  if(canEmergency){
    ...
    ROS_INFO("slowly dacc");
    give_velocity(v_now, w_now);
  }
  last_time = ros::Time::now();
}
...
float pose_tracker::compensate_angular_vel(){//要重新整理
  if( linear_compensate ){
    ...
    else if(temp_angle > M_PI && temp_angle <= (2*M_PI+0.001)){
      //逆轉
      temp_angle = temp_angle - 2*M_PI;
    }

    w_now = (Kd_*distance_ + Kt_*temp_angle)*fabs(v_now);
    //確保不要爆衝
    ...
    if(fabs(w_now) >= fabs(w_max)){
      return w_max*w_now/fabs(w_now);
    }
...
float pose_tracker::linear_velocity_profile(float difference_distance){
  ...
  if (decelerate_mode){
    ...
    if( fabs(new_v_dacc)>= (fabs(v_dacc)+v_dacc_tolerance) || fabs(new_v_dacc)<= (fabs(v_dacc)-v_dacc_tolerance) ){
      new_v_dacc = v_dacc;
    }
...
float pose_tracker::angular_velocity_profile(float difference_angle){
  ...
  if (decelerate_mode){
    ...
    if( fabs(new_w_dacc)>= fabs(w_dacc)){
      new_w_dacc = w_dacc;
    }
